...
import { useLocalVideo, useLocalAudio,usePeerIds  } from '@huddle01/react/hooks';
...
const Huddle = () => {
  ...
  const {peerIds}= usePeerIds({ roles: [Role.HOST, Role.CO_HOST] });
    ...
    create a room 
      ...
      <button className='bg-blue-400 p-3'   onClick={() => joinRoom({roomId:roomid,token}) } >
        Join Lobby
      </button>
...
{    isVideoOn && <video ref={videoRef} autoPlay muted className='w-1/2 mx-auto border-2 rounded-xl border-blue-400' />
